    ...
    async def save(self, paste_id: str, content: str, meta: Dict[str, Any]):
        ...
        await loop.run_in_executor(
            ...
            lambda: self.s3.put_object(
                Bucket=self.bucket,
                Key=self._key(paste_id, "content.txt"),
        ...
        await loop.run_in_executor(
            ...
            lambda: self.s3.put_object(
                Bucket=self.bucket,
                Key=self._key(paste_id, "meta.json"),
        ...
        try:
            obj = await loop.run_in_executor(
                ...
                lambda: self.s3.get_object(
                    Bucket=self.bucket, Key=self._key(paste_id, "content.txt")
                ),
            )
            content = obj["Body"].read().decode("utf-8")
            try:
                mobj = await loop.run_in_executor(
                    None,
                    lambda: self.s3.get_object(
                        Bucket=self.bucket, Key=self._key(paste_id, "meta.json")
                    ),
                )
